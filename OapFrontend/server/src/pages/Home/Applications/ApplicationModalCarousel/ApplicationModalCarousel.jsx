...
const ApplicationModalCarousel = ({ items = [], selectedItem, onItemClick, loading }) => {
  ...
  buildRef.current = () => {
    ...
    if (loading) {
      viewport.className = "appHome-carousel-viewport appHome-carousel-viewport--static";
      const track = document.createElement("div");
      track.className = "appHome-carousel-track";
      for (let i = 0; i < 4; i++) {
        const s = document.createElement("div");
        s.className = "appHome-carousel-img appHome-carousel-skeleton";
        track.appendChild(s);
      }
    ...
    if (its.length <= 1) {
      ...
      g.className = "appHome-carousel-solo-gradient";
      viewport.appendChild(g);
      return;
    }

    viewport.className = "appHome-carousel-viewport appHome-carousel-viewport--live";
    ...
    for (let i = 0; i < needed; i++) {
      ...
      el.className = "appHome-carousel-img" +
        (item.fileId === selectedFileIdRef.current ? " appHome-carousel-img--selected" : "");
      el.style.cssText =
        `position:absolute;` +
    ...
    const tick = (ts) => {
      ...
      if (last.pos > vpSz) {
        ...
        node.el.style[prop] = `${node.pos}px`;
        node.el.classList.toggle("appHome-carousel-img--selected", it.fileId === selectedFileIdRef.current);
        pool.unshift(node);
      }
  ...
  useEffect(() => {
    ...
    roRef.current = new ResizeObserver(() => {
      ...
      if (!vp) return;
      if ((vp.offsetWidth > 0 || vp.offsetHeight > 0) && itemsRef.current.length >= 2 && !loading)
        buildRef.current();
    });
  ...
  useEffect(() => { buildRef.current(); }, [loading]);
  ...
  return (
    <section className="appHome-carousel-section">
      <div ref={viewportRef} className="appHome-carousel-viewport appHome-carousel-viewport--static" />
    </section>
  );
};
